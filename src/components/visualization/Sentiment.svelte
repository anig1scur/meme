<script>
  ...
  const xKey = ['humour', 'sarcasm', 'offensive', 'motivation', 'overall'];
  const itemsPerPage = 6;
  const totalPages = 7;
  let currentPage = 0;

  const data = raw.slice(0, itemsPerPage * totalPages).map((d) => {
    const [humour, sarcasm, offensive, motivation] = d.emotion;
    const overall = (humour + motivation - sarcasm - offensive + 10) / 4;

    return {...d, humour, sarcasm, offensive, motivation, overall};
  });
...
<section class="visualization">
  <div class="title">Emotion Radar of Memes</div>
  <div class="font-routedgothic grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 mb-12">
