...
<div
  ...
  class="fixed right-5 top-1/2 -translate-y-1/2 h-4/5 w-16 flex flex-col items-center translate-x-full opacity-0"
>
  ...
  {#each timelineData as segment}
    <div
      class="absolute left-5 w-full -translate-y-1/2 flex items-center cursor-pointer transition-all duration-300 ease-in-out"
      style="top: {segment.position}%"
      on:click={() => handleSegmentClick(segment)}
    >
      <div
        class={`w-2 h-2 rounded-full mx-2 transition-all duration-300 ease-in-out ${segment.id === activeSegment ? 'w-3 h-3 bg-purple-600 mx-[5px]' : 'bg-gray-500'}`}
      ></div>
      <div
